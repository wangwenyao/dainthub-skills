...
- ❌ 禁止同一页面出现 2 个以上视觉权重相等的元素（C-HIER-001）
- ❌ 禁止用颜色作为唯一层级区分手段（C-HIER-002）
- ❌ 禁止三级信息使用深色或粗体（C-HIER-003）
- ❌ 禁止表格行内出现 3 种以上字号（C-HIER-004）
- ❌ 禁止卡片嵌套超过 2 层（C-HIER-005）
- ❌ 禁止用边框粗细区分层级，用颜色明度（C-HIER-006）
