...
| F0106 | 批量删除 | P1 | F01 商品列表 | 确认弹窗 | 管理员 |
...
每个二级功能（P0/P1）用以下模板展开概览，三级功能点在页面线框图中体现。
...
**三级功能点详细描述模板**（针对复杂功能点展开）：

对于复杂功能点（如审批、复杂表单），需展开详细描述。简单的 CRUD 功能点可在页面线框图中体现，无需单独展开。
...
- BR01: 名称不可与现有商品重复
- BR02: 价格为 0 时需提示确认
- BR03: 库存为 0 时状态默认为"下架"

**异常处理**
| 异常场景 | 提示文案 | 处理方式 |
|----------|----------|----------|
| 名称重复 | "名称已存在，请修改后重试" | 阻止提交，高亮字段 |
| 价格为0 | "价格为0，确认继续？" | 弹窗二次确认 |
| 无权限 | "您没有权限执行此操作" | 按钮置灰或隐藏 |

**验收标准**
- Given 用户已登录且有新增权限 When 点击新增按钮 Then 打开表单弹窗
- Given 表单填写完成 When 点击确定 Then 校验通过后保存数据，弹窗关闭，列表刷新
- Given 名称已存在 When 点击确定 Then 提示"名称已存在"，阻止提交
```

> **使用规则**：
> - 简单 CRUD 功能点（查看、搜索）：无需单独展开，在页面线框图中体现即可
> - 新增/编辑功能点：需填写字段约束和业务规则
> - 删除功能点：只需填写确认文案和权限规则，无需复杂展开
> - 复杂功能点（审批、多步骤流程）：需完整填写交互流程和业务规则
