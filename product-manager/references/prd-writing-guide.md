...
| 复杂度 | 定义 | 描述方式 | Token预估 |
|--------|------|----------|-----------|
| **简单** | 单一步骤，无分支逻辑 | 一句话描述 | ~100 |
| **标准** | 3-5 步骤，有标准校验规则 | 引用模板 + 差异标注 | ~300 |
| **复杂** | ≥5 步骤，有复杂业务规则/状态流转/多角色协作 | 完整描述 | ~800 |
...
**引用 `@prd-template.md#通用功能模板库` 减少重复内容**：
...
| | | F0103 新增 | P0 | P002 表单弹窗 | 管理员 | A02 商品上架 |
...
| | | F0203 编辑分类 | P0 | P004 分类表单 | 管理员 | A05 分类管理 |
| | | F0204 删除分类 | P1 | 确认弹窗 | 管理员 | A05 分类管理 |
| M02 订单管理 | F03 订单列表 | F0301 查看列表 | P0 | P005 列表页 | 管理员/客服 | A06 订单查询 |
| | | F0302 搜索订单 | P0 | P005 搜索区 | 管理员/客服 | A06 订单查询 |
| | | F0303 导出订单 | P1 | toolbarActions | 管理员 | A07 订单导出 |
| | F04 订单详情 | F0401 查看详情 | P1 | P006 详情页 | 管理员/客服 | A08 订单查看 |
| | | F0402 取消订单 | P1 | 操作按钮 | 管理员 | A09 订单取消 |
