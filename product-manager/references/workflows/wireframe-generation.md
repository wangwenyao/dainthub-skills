...
2. 验证检查项：
   ├── ☐ **功能点→页面映射验证表是否无缺失？**
   ├── ☐ **逐项生成清单是否逐项输出？**
   ├── ☐ HTML 是否使用模板定义的 class？
   ├── ☐ HTML 是否使用灰色系配色？（无真实颜色）
   ├── ☐ YAML 是否包含完整的 pages 配置？
   ├── ☐ 每个 page 是否有 type、route、config？
   ├── ☐ 是否按一级模块分开生成文件？
...
- v2.0
- 更新: 2026-04-12
- 新增: 功能点→页面映射验证表（数量传递约束）
- 新增: 逐项生成约束（必须逐项输出）
- 新增: 硬约束章节
- 新增: 执行约束章节，强制输出格式要求和禁止行为