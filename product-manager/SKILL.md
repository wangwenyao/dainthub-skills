...
**根据端划分自动选择文档结构，控制 Token 消耗**：

| 端数量 | 文档结构 | Token 控制 | 适用场景 |
|--------|----------|------------|----------|
| ≥ 2 个端 | **三层结构** | 总览 ≤2000 + 各端 ≤5000 | 小程序 + 管理后台 |
| = 1 + 模块 ≥ 3 | **两层结构** | ≤6000 | 管理后台多模块 |
| = 1 + 模块 ≤ 2 | **单层结构** | ≤3000 | 简单管理后台 |
...
|--------|------|----------|-----------|
| 简单 | 单一步骤，无分支 | 一句话 | ~100 |
| 标准 | 3-5 步骤，有校验 | 引用模板 + 差异 | ~300 |
| 复杂 | ≥5 步骤，复杂规则 | 完整描述 | ~800 |

**通用功能模板引用**（减少 Token）：
| 模板ID | 适用功能 | Token节省 |
|--------|----------|-----------|
| T01-T06 | 查看/搜索/新增/编辑/删除/导出 | ~60-80% |
