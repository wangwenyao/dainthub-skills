...
| **建模阶段** | 业务建模流程 | 必选 | 角色、流程、活动、规则定义 |
| **架构阶段** | 功能架构设计流程 | 必选 | 三级功能点拆解 |
| **文档阶段** | 流程三：PRD编写 | 必选 | 完整PRD文档 |
| **文档阶段** | 线框图生成流程 | 必选 | HTML + YAML输出 |
| **验证阶段** | 原型走查验证 | 必选 | 线框图走查 + 流程模拟 |
| **评审阶段** | 流程四：需求评审 | 必选 | 完整性/清晰度检查 |
| **迭代阶段** | 流程五：偏差反馈处理 | 可选 | 如有偏差才执行 |
...
| 用户意图 | 执行流程 | 立即读取 |
|---------|---------|---------|
| 写PRD / 写需求文档 | 流程三：PRD编写 | `references/workflows/prd-writing.md` |
| 生成线框图 / 页面原型 | 线框图生成流程 | `references/workflows/wireframe-generation.md` |
| 评审PRD / 检查需求 | 流程四：需求评审 | `references/workflows/review.md` |
...
| 设计功能架构 / 功能拆解 | 功能架构设计流程 | `references/workflows/function-architecture-design.md` |
| 业务建模 / 角色/流程定义 | 业务建模流程 | `references/workflows/business-modeling-flow.md` |
| 处理偏差反馈 | 流程五：偏差反馈处理 | `references/workflows/deviation-handling.md` |
| 原型走查 / 验证流程 | 原型走查验证 | `references/workflows/prototype-validation.md` |
...
**输出双轨格式：**
- **wireframes.html**：HTML+CSS 线框图（产品经理review）
