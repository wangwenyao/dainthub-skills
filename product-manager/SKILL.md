...
description: Use when 企业信息系统（业务驱动）需要写PRD、写需求文档、功能设计、产品规划、用户故事，或提到：帮我梳理需求、整理功能点、写产品方案、这个功能怎么设计、帮我想想产品逻辑、产品需求文档、生成线框图、页面原型、设计页面结构、处理偏差反馈、技术反馈、偏差决策、接受技术建议。不适用：UI 视觉设计（由 saas-ui-design 处理）、技术实现方案（由 tech-manager 处理）、纯代码开发、C端消费者产品、竞品分析。
---
...
| 验证时机 | 必须动作 |
|----------|----------|
| **生成前** | 确认已读取所有需要的 references 文件 |
| **生成中** | 每完成一个章节，对照规范检查格式 |
| **生成后** | 检查输出完整性：必需章节完整、格式符合规范、无遗漏项 |
...
以下行为表明 skill 执行失败：
- 输出的功能清单没有使用统一表格格式（一级合并、二级合并、三级按行）
- 输出的功能清单缺少「对应业务活动」列，无法追溯业务来源
- 输出的功能详细描述只写了 P0，遗漏了 P1 功能
- 输出的线框图使用了真实颜色或图标
...
| UX五要素范围层 | 功能规格 | 8.1 统一功能清单表 |
| UX五要素结构层 | 信息架构 | 8.2 功能菜单层级结构 |
...
- v5.0
- 更新: 2026-04-12
- 补充: 功能清单增加「对应业务活动」列，实现业务↔功能双向追溯
- 删除: 需求评审流程和原型走查验证流程（保留质量门控）
- 重构: 功能清单格式改为统一表格（一级合并、二级合并、三级按行展示）
- 扩展: 功能详细描述范围从 P0 扩展到 P0/P1