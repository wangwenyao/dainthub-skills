...
| 业务域 | 功能模块 | 主要功能点 | 优先级 | 本期范围 |
|---|---|---|---|---|
| 系统管理域 | 用户管理 | 用户新增/编辑/禁用、密码重置、登录日志查询 | P0 | ✓ 本期实现 |
| 系统管理域 | 角色权限 | 角色配置、菜单权限分配、按钮级权限控制 | P0 | ✓ 本期实现 |
| {核心业务域} | {模块名} | {功能点1}、{功能点2}（3-6 个，含隐性需求推导出的功能点）| P0/P1/P2 | ✓ 本期实现 / ○ 后续迭代 |
| 分析报表域 | {模块名} | {功能点} | P1/P2 | ○ 后续迭代 |

**列填充规则：**
- **优先级**：P0 主业务流程上的功能（缺少则系统无法运行）；P1 核心业务功能（一期完成）；P2 辅助/优化类功能（可后续迭代）
- **本期范围**：仅当客户明确"本期/后续"分期交付时填写；如未分期，整列可省略
...
**Mermaid 语法规范（流程图与系统架构图通用）：**

- **节点 ID**：只用英文字母和数字（如 `A`、`node1`），不用中文或特殊字符
- **节点命名**：
  - 操作节点用动宾结构（如 `[填写申请单]`）
  - 判断节点用疑问句（如 `{金额是否超限}`）
  - 开始/结束用体育场形（如 `([流程开始])`）
- **节点形状**：`[]` 矩形 / `()` 圆角矩形 / `{}` 菱形 / `(())` 圆形 / `[()]` 体育场形
- **连接线**：标注文字不超过 10 个字（如 `-- 通过 -->`）
- **subgraph 语法**：`subgraph 名称` 开头，`end` 结尾
- **节点数量**：单张流程图节点数不超过 15 个，超过时分拆为多张图
- **图前后说明**：每张图前写一句引导语，图后写文字补充图中无法完整表达的内容
