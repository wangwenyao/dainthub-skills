...
| 生成接口文档 / OpenAPI / 接口规格 | 接口规格生成 | `workflows/interface-spec-generation.md` |
| 生成数据库脚本 / DML / SQL脚本 | DML脚本生成 | `workflows/dml-script-generation.md` |
...
| git pull / 拉取同步 / 检查变更 | 拉取同步流程 | `../docs/collaboration/06-sync-flow.md` |
| 项目初始化 / 协同开发 / 分布式协作 | 协同开发概述 | `../docs/collaboration/01-overview.md` |
| 目录结构 / 项目结构 | 项目目录结构 | `../docs/collaboration/02-project-structure.md` |
| status.md / 归档 / 状态更新 | 状态快照与归档 | `../docs/collaboration/05-status-mechanism.md` |
...
（执行 DML + 读取 OpenAPI）  （读取 OpenAPI → API调用）
       ↓                         ↓
       └────────────┬────────────┘
                    ↓
             tech-manager 实现验证
...
| 文档 | 用途 | 何时加载 |
|------|------|----------|
| `../docs/collaboration/01-overview.md` | 协同开发概述 + 触发指令 | 项目初始化 |
| `../docs/collaboration/05-status-mechanism.md` | status.md 状态快照 + 归档机制 | 更新状态/归档 |
| `../docs/collaboration/06-sync-flow.md` | 拉取同步流程 | git pull 后 |
| `../docs/collaboration/02-project-structure.md` | 项目目录结构 | 需要了解目录时 |
...
- 新增: 分布式协同开发指南引用（docs/collaboration-guide.md）
- 新增: 参考文档索引表
- 新增: 项目初始化/协同开发快速路由
- 新增: 接口规格生成能力（OpenAPI 3.0 规范）
- 新增: DML脚本生成能力（含回滚脚本、执行清单、变更日志）
- 新增: db_change_log 变更追踪机制
- 优化: 核心能力从五大扩展为六大
- 优化: 协作链路明确 OpenAPI + DML 输出物